...
st.latex(r'''
    \delta = \frac{\text{Industry CO2 Emissions(tn)} \times \text{ETS Price(USD/tn)}}{\text{Industry Output, USD}} \times \text{Tax rate}
    ''')
...
with st.expander("Source description"):
    st.write("It presents the breakdown of output at basic prices between market output, output for own final use and non-market output, by activty at the 2 digit ISIC Rev 4 level. It has been prepared from statistics reported to the OECD by countries in their answers to Supply and Use questionnaire.")
